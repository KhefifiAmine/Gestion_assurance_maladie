...
const login = async (req, res) => {
    try {
        ...
        if (req.body.isAdminLogin !== true && ['ADMIN', 'RESPONSABLE_RH'].includes(user.role)) {
            return res.status(403).json({ message: 'Veuillez utiliser le portail administratif pour vous connecter.' });
        }

        // Checking status BEFORE password validation
        if (user.statut === 0) {
        ...
        // Now we know the account is active, check if it has a password set
