...
const reclamationController = {
  ...
  delete: async (req, res) => {
    try {
      ...
      // SEUL l'adhérent peut supprimer sa propre réclamation, et uniquement si elle est 'Ouverte'
      // L'admin et le RH ne peuvent JAMAIS supprimer une réclamation (pour archivage/traçabilité)
      if (userRole !== 'ADHERENT') {
        return res.status(403).json({ 
          success: false, 
          message: 'Action non autorisée : Seul l\'adhérent peut supprimer sa propre réclamation.' 
        });
      }
