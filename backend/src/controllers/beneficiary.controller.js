...
const addBeneficiary = async (req, res) => {
    try {
        ...
        if (req.files && req.files.length > 0) {
            documentPath = JSON.stringify(req.files.map(f => f.filename));
        }
...
const deleteBeneficiary = async (req, res) => {
    try {
        ...
        if (beneficiary.document) {
            try {
                const files = JSON.parse(beneficiary.document);
                if (Array.isArray(files)) {
                    ...
                    // Fallback pour les anciens enregistrements (string simple)
                ...
                // Fallback si le parse JSON échoue (ancien format)
                const filePath = path.join(__dirname, '../../uploads', beneficiary.document);
                if (fs.existsSync(filePath)) {
                    await fs.promises.unlink(filePath);
                }
...
const updateBeneficiary = async (req, res) => {
    try {
        ...
        if (req.files && req.files.length > 0) {
            ...
            if (beneficiary.document) {
                try {
                    const oldFiles = JSON.parse(beneficiary.document);
