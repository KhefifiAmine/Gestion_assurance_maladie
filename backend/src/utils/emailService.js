...
console.log("📂 [Boot] Chargement du service Email (Nodemailer)...");
...
if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
  console.warn("⚠️ [Email Service] ATTENTION: EMAIL_USER ou EMAIL_PASS n'est pas défini dans l'environnement !");
}

const sendResetEmail = async (email, code) => {
  const mailOptions = {
    from: `"CareCover - Notifications" <${process.env.EMAIL_USER || 'no-reply@carecover.com'}>`,
    to: email,
...
const sendBlockEmail = async (email, objet, raison = "") => {
  const mailOptions = {
    from: `"CareCover - Sécurité" <${process.env.EMAIL_USER || 'no-reply@carecover.com'}>`,
    to: email,
