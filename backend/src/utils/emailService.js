...
if (!process.env.RESEND_API_KEY) {
  console.warn("⚠️ [Email Service] ATTENTION: RESEND_API_KEY n'est pas défini dans l'environnement !");
}

// Resend nécessite souvent une adresse d'expédition vérifiée. 
// Par défaut pour le mode test : onboarding@resend.dev
const DEFAULT_FROM = process.env.EMAIL_FROM || 'CareCover <onboarding@resend.dev>';
...
const sendResetEmail = async (email, code) => {
  try {
    ...
    if (error) {
      console.error('❌ [Email Service] Erreur Resend:', error);
      return false;
    }
...
const sendApprovalEmail = async (email) => {
  try {
    ...
    console.error('❌ [Email Service] Erreur envoi email d\'approbation:', error);
    return false;
  }
};
...
const sendRejectionEmail = async (email, objet, raison = "") => {
  try {
    ...
    console.error('❌ [Email Service] Erreur envoi email de refus:', error);
    return false;
  }
};
...
const sendBlockEmail = async (email, objet, raison = "") => {
  try {
    ...
    console.error('❌ [Email Service] Erreur envoi email de blocage:', error);
    return false;
  }
};
...
const sendNotificationEmail = async (email, titre, description) => {
  try {
    ...
    console.error('❌ [Email Service] Erreur envoi email notification:', error);
    return false;
  }
};
...
const sendRateLimitAlertEmail = async (superAdminEmail, ip, limiterName, path) => {
  ...
  try {
    if (!process.env.RESEND_API_KEY) {
      console.log(`--- MOCK EMAIL (RATE LIMIT) ---`);
      console.log(`To: ${superAdminEmail}`);
      console.log(`IP: ${ip} | Limiter: ${limiterName} | Route: ${path}`);
      return true;
    }
    ...
    console.error('❌ [Email Service] Erreur envoi alerte rate limit:', error);
    return false;
  }
};
