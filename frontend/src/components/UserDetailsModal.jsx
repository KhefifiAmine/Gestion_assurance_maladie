...
const UserDetailsModal = ({ isOpen, onClose, user }) => {
    ...
    if (user.statut === 2 || user.statut === 3) {
        if (user.objet_blocage) {
            detailsFields.push({
                label: user.statut === 2 ? 'Objet du refus' : 'Objet du blocage',
                value: user.objet_blocage,
                icon: Info
            });
        }
        if (user.motif_blocage) {
            detailsFields.push({
                label: user.statut === 2 ? 'Motif du refus' : 'Motif du blocage',
                value: user.motif_blocage,
                icon: Info
            });
        }
    }
