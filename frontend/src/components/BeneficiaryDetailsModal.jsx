                    ...
                    <div className="flex-1 flex flex-col max-h-[85vh] overflow-y-auto custom-scrollbar">
                        ...
                        <div className="p-8 space-y-6">
                            ...
                            {beneficiary.document && (
                                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50 space-y-4 lg:hidden">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-xl bg-indigo-100 text-indigo-600 flex items-center justify-center">
                                            <FileText size={18} />
                                        </div>
                                        <div>
                                            <div className="text-xs font-bold text-slate-800 dark:text-slate-200">Justificatif(s) fourni(s)</div>
                                            <div className="text-[10px] font-medium text-slate-500 uppercase tracking-widest">Documents rattachés</div>
                                        </div>
                                    </div>
                                    <div className="flex flex-wrap gap-2">
                                        {(() => {
                                            ...
                                            return files.map((file, idx) => (
                                                <button
                                                    ...
                                                    className="flex-1 min-w-[120px] px-4 py-2 rounded-xl bg-white dark:bg-slate-700 text-indigo-600 hover:bg-indigo-600 hover:text-white border border-slate-200 dark:border-slate-600 hover:border-indigo-600 transition-all text-[10px] font-bold uppercase tracking-widest flex items-center justify-center gap-2"
                                                >
                                                    <Eye size={12} /> Doc #{idx + 1}
                                                </button>
                                            ));
                                        })()}
                                    </div>
                        ...
                        <div className="hidden lg:flex w-[450px] bg-slate-50 dark:bg-slate-950/50 p-10 flex-col gap-8 border-l border-slate-100 dark:border-white/5 relative shrink-0">
                            ...
                            <div className="relative z-10 flex flex-col h-full overflow-hidden">
                                <div className="flex items-center gap-3 mb-8">
                                    ...
                                    <div>
                                        <h4 className="text-xs font-black uppercase tracking-widest text-slate-500">Documents Justificatifs</h4>
                                        ...
                                        return files.map((file, idx) => (
                                            <div key={idx} className="space-y-3">
                                                <div className="flex items-center justify-between px-2">
                                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Justificatif #{idx + 1}</p>
                                                    <button 
                                                        onClick={() => onPreviewDocument(file)}
                                                ...
                                                <div className="w-full aspect-[3/4] rounded-[32px] overflow-hidden border-4 border-white dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl relative group">
                                                    {file.toLowerCase().endsWith('.pdf') ? (
                                                        <iframe src={`http://localhost:5000/uploads/${file}`} className="w-full h-full border-none" title={`Preview-${idx}`} />
                                                    ) : (
                                                        <img src={`http://localhost:5000/uploads/${file}`} alt={`Preview-${idx}`} className="w-full h-full object-contain" />
                                                    )}
                                                </div>
                                            </div>
                                        ));
