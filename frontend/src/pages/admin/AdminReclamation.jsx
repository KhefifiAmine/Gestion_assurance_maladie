...
//import AdherentReclamationList from '../../components/AdherentReclamationList';
...
const AdminTable = ({ reclamations, onView, showConfirm, showToast, onActionSuccess }) => {
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {filtered.map((r) => (
                <tr key={r.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-8 py-7 dark:text-slate-300 font-medium">{r.createdAt ? new Date(r.createdAt).toLocaleDateString('fr-FR') : 'N/A'}</td>
                  <td className="px-8 py-7 font-bold dark:text-white">{r.adherentNom || 'N/A'}</td>
                    ...
                    <div className="flex justify-center gap-3">
                      <button 
                        onClick={() => showConfirm(
                          ...
                          async () => {
                            try {
                              await deleteReclamation(r.id);
                              onActionSuccess(r.id, 'delete');
                              showToast('Dossier supprimé', 'success');
                            } catch (err) { showToast(err.message, 'error'); }
                          },
                          'danger'
                        )}
                        className="p-3 bg-red-50 text-red-600 rounded-2xl hover:bg-red-600 hover:text-white opacity-0 group-hover:opacity-100 transition-all"
                      ><Trash2 size={18} /></button>
                      <button onClick={() => onView(r.id)} className="p-3 bg-slate-900 text-white rounded-2xl hover:bg-purple-600 transition-all"><Eye size={18} /></button>
                    </div>
                  </td>
                </tr>
              ))}
