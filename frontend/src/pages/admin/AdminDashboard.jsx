...
const AdminDashboard = ({ mode = 'all' }) => {
    ...
    const [activeRoleFilter, setActiveRoleFilter] = useState(mode === 'all' ? 'Tous' : mode === 'adherents' ? 'Adhérents' : mode === 'admins' ? 'Administrateurs' : 'Tous');
    ...
    const handleRoleChange = async (id, currentRole) => {
        setModalConfig({
            ...
            message: `Veuillez sélectionner le nouveau rôle pour cet utilisateur.`,
            requireRoleSelect: true,
    ...
    const handleConfirmAction = async (payload) => {
        const { userId, type, newStatus } = modalConfig;
        setModalConfig(prev => ({ ...prev, isOpen: false }));

        try {
            if (type === 'role') {
                await updateUserRole(userId, payload);
                showToast(`Rôle mis à jour : ${payload}`, "info");
            } else if (type === 'status') {
                await updateUserStatus(userId, newStatus, payload);
    ...
    const filteredUsers = useMemo(() => {
        ...
        return users.filter(user => {
            ...
            if (activeRoleFilter === 'Adhérents') {
                matchesRole = (userRole === 'ADHERENT');
            } else if (activeRoleFilter === 'Administrateurs') {
                matchesRole = (userRole === 'ADMIN');
            } else if (activeRoleFilter === 'Responsable RH') {
                matchesRole = (userRole === 'RESPONSABLE_RH');
            }
    ...
    }, [users, searchTerm, activeFilter, activeRoleFilter]);
            ...
            <div className="max-w-7xl mx-auto space-y-8">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-slate-500 dark:text-slate-400 font-medium tracking-tight">
                                Consulter et gérez les comptes de tous les utilisateurs (Adhérents, Admins, RH).
                            </p>
                        </div>
                    </div>

                    {/* Button Ajouter Utilisateur - Visible seulement pour l'Admin */}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <motion.div
                        ...
                        onClick={() => setActiveRoleFilter('Tous')}
                    >
                        <div>
                    ...
                    <motion.div
                        ...
                        className="bg-white dark:bg-slate-900 p-8 rounded-[32px] shadow-sm border border-slate-100 dark:border-slate-800 flex items-center justify-between group hover:shadow-xl transition-all duration-500 cursor-pointer"
                        onClick={() => setActiveRoleFilter('Adhérents')}
                    >
                        <div>
                    ...
                    <motion.div
                        ...
                        className="bg-white dark:bg-slate-900 p-8 rounded-[32px] shadow-sm border border-slate-100 dark:border-slate-800 flex items-center justify-between group hover:shadow-xl transition-all duration-500 cursor-pointer"
                        onClick={() => setActiveRoleFilter('Administrateurs')}
                    >
                        <div>
                    ...
                    <motion.div
                        ...
                        className="bg-white dark:bg-slate-900 p-8 rounded-[32px] shadow-sm border border-slate-100 dark:border-slate-800 flex items-center justify-between group hover:shadow-xl transition-all duration-500 cursor-pointer"
                        onClick={() => setActiveRoleFilter('Responsable RH')}
                    >
                        <div>
                ...
                <motion.div
                    ...
                    className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-white/5 flex flex-col gap-6"
                >
                    ...
                    <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-slate-50 dark:border-slate-800">
                        <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mr-2">Filtrer par Rôle :</span>
                        {['Tous', 'Adhérents', 'Administrateurs', 'Responsable RH'].map((role) => (
                            <button
                                key={role}
                                onClick={() => setActiveRoleFilter(role)}
                                className={`px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border ${activeRoleFilter === role
                                    ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-500/20'
                                    : 'bg-transparent text-slate-400 dark:text-slate-500 border-slate-100 dark:border-slate-800 hover:border-indigo-500/50'
                                    }`}
                            >
                                {role}
                            </button>
