...
import {
  ...
  ArrowLeft, Clock, ShieldAlert, ChevronRight, ChevronDown, Bell, RefreshCw, Trash2, MessageSquare
} from 'lucide-react';
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="pt-4">
            ...
            >
              ...
              {bulletins.map(b => (
                <option key={b.id} value={b.id}>#{b.numero_bulletin}</option>
              ))}
            </select>
            {errors.bulletinId && <p className="text-red-500 text-[10px] font-black mt-2 px-1 uppercase">{errors.bulletinId}</p>}
          </div>
            ...
            <table className="w-full border-separate border-spacing-y-3 px-8">
              <thead>
                <tr className="text-slate-400 dark:text-slate-500">
                  <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-left">Référence</th>
                  <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-left">Date Dépôt</th>
                  <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-left">Objet / Raison</th>
              ...
              <tbody>
                {reclamations.map((r, idx) => (
                  ...
                  >
                    ...
                    <td className="bg-white dark:bg-slate-900 border-y border-slate-100 dark:border-white/5 px-6 py-5 group-hover:bg-slate-50/50 dark:group-hover:bg-white/[0.02] transition-colors">
                        <div className="flex flex-col">
                            <span className="text-[11px] font-black text-slate-900 dark:text-white uppercase tracking-tight">
                                {r.createdAt ? new Date(r.createdAt).toLocaleDateString('fr-FR') : 'N/A'}
                            </span>
                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Dépôt Dossier</span>
                        </div>
                    </td>
                    ...
                    <td className="bg-white dark:bg-slate-900 border-y border-slate-100 dark:border-white/5 px-6 py-5 group-hover:bg-slate-50/50 dark:group-hover:bg-white/[0.02] transition-colors">
                        <div className="flex flex-col">
                            <span className="font-bold text-slate-800 dark:text-slate-200 tracking-tight line-clamp-1">
                                {r.objet}
                            </span>
                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider mt-1">Sujet Principal</span>
                        </div>
                    </td>
                    ...
                    <td className="bg-white dark:bg-slate-900 last:rounded-r-[2rem] border-y border-r border-slate-100 dark:border-white/5 px-6 py-5 group-hover:bg-slate-50/50 dark:group-hover:bg-white/[0.02] transition-colors text-center">
                        <div className="flex items-center justify-center gap-2">
                            ...
                            <div className="p-2.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl hover:bg-purple-600 dark:hover:bg-purple-500 hover:text-white transition-all shadow-sm">
...
const UserReclamation = () => {
  ...
  const handleDelete = (id) => {
    ...
    if (rec && rec.statut !== 'Ouverte') {
      showToast('Impossible de supprimer une réclamation non ouverte.', 'warning');
      return;
    }
