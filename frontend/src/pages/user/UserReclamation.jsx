            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5">
                  ...
                  {(user?.role === 'ADMIN' || user?.role === 'RESPONSABLE_RH') && <th className="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Priorité</th>}
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                {reclamations.map((r) => (
                  <tr key={r.id} onClick={() => onView(r.id)} className={`transition-all duration-300 cursor-pointer group relative border-l-[6px] ${r.statut === 'Ouverte' ? 'border-purple-500' : r.statut === 'En cours' ? 'border-amber-500' : r.statut === 'Traitée' ? 'border-emerald-500' : 'border-slate-400'} hover:bg-slate-50 dark:hover:bg-slate-800/50`}>
                    ...
                    {(user?.role === 'ADMIN' || user?.role === 'RESPONSABLE_RH') && <td className="p-6"><PriorityBadge priorite={r.priorite} /></td>}
                      ...
                      <div className="flex items-center justify-end gap-3 px-1">
                        {r.statut === 'Ouverte' && (
                          <button
                            ...
                            className="p-3 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 transition-all hover:bg-red-600 hover:text-white shadow-sm"
